...
const Wishlist = () => {
  ...
  // console.log("wishli",wishlistData);
  ...
  const handledeleteWishlist = async (filterwishlist,wishlistid,product) =>{
    console.log("deletewishlist",filterwishlist,wishlistId1,[...wishlistId1,product?.product_id]);
    ...
    await axios.post(`${BASE_URL}/delete-wishlist`,{"wishlist_id":filterwishlist[0]?.wishlist_id},{
      headers:{
      ...
      const updatedWishlist = wishlistId1.filter(id => id !== product?.product_id);
      const filterData = wishlistData.filter((i)=>i.product_id !== product?.product_id);
      // const updateWishlist = [...wishlistId,product?.product_id];
      console.log("updatewishlist",updatedWishlist,"filterData",filterData);
  ...
  const handleDeleteWishlist = async (e,product) =>{
    ...
    console.log("delte",a);
    await axios.post(`${BASE_URL}/delete-wishlist`,{"wishlist_id":a[0]?.wishlist_id},{
      headers:{
        Authorization: `Bearer ${token}`.replace(/\s+/g, " ").trim(),
        "Content-Type": "application/json",
      }
  ...
  const handleAddToCart = async (e,product,selectedAttributes,qty) =>{
     if (!token) { toast.error("Please login to add product to wishlist!")}
    // console.log("addtoCart",product,"cartData",cartData);
    const filterwishlist = wishlistData?.filter((i)=>i?.product_id === product?.product_id);
    ...
    const AlreadyExistsData = cartData?.items.filter((i)=> {
      const sizeMatch = i?.variation_id !== 0 ? i?.variation_id === selectedAttributes?.id : true && i?.product_id === product?.product_id
      return sizeMatch;
    });
    console.log("AlreasyExistData",AlreadyExistsData,"quantity",qty,"selectAttributes",selectedAttributes);
        ...
        if(product?.variations  && product?.variations.length !== 0 && selectedAttributes === 0){
          ...
          if(selectedAttributes === null && selectedAttributes !== undefined){
            ...
            const payload = {
              ...
              variation_id:selectedAttributes === 0 ? selectedAttributes : selectedAttributes?.id,
              quantity:qty
            }
            ...
            await axios.post(`${BASE_URL}/new-add-to-cart`, payload,{
              ...
              dispatch(AddToCart({...res.data,items:[...cartData.items,res.data?.data]}));
            })
  ...
  return (
    <div className="home-main wishlist-main">
      ...
      {
        loading || Loading ? <Loader1></Loader1> : <>
            {
                  ...
                  return (
                    <React.Fragment key={index}>
                      <div className="wishlist-item" key={index}>
                        ...
                        <button className="btn wishlist-addTocart" onClick={(e)=>handleAddToCart(e,item,0,1)}>
                          Add to Cart
                        </button>
                        <button className="wishlist-item-remove" onClick={(e)=>handleDeleteWishlist(e,item)}>✕</button>
                      </div>
                    </React.Fragment>
                  );
                })}
